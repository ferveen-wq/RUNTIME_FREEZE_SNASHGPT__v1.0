# CERAMIC — PHASE 3B EVIDENCE AUDIT
...
### Readiness gate
- Phase 3B readiness for CERAMIC is defined in PHASE3A_QUALIFICATION_DECISION_MATRIX.md:
...
- For CERAMIC price-ready path:
  - PHASE3B_CERAMIC_RANGE is the mapped phrase block
- runner/context_reset_prompt.txt hardens the CERAMIC price-ready path:
  - selected_phrase_id MUST equal PHASE3B_CERAMIC_RANGE
  - override must not apply unless QUALIFICATION_STATUS == READY_FOR_NEGOTIATION
- Tested-lane enforcement remains stricter than engine tolerance where READY_FOR_NEGOTIATION is required in the hardened path
...
- No additional git recovery required for this first CERAMIC Phase 3B pass
...
- rendered through PHASE4_8_MESSAGE_ASSEMBLY_MAP.md using PHASE3B_CERAMIC_RANGE for the ceramic price-ready path
- constrained by the same broader READY vs READY_FOR_NEGOTIATION reconciliation point already known in Phase 3B
